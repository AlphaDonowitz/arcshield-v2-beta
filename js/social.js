...
// LISTAS
window.loadTokenExplorer = async function() { /* (Já coberto pelo loadMyPortfolio e subTabs) */ }
...
window.loadLeaderboard = async function() {
    ...
    let html = ""; 
    u.forEach((x, i) => { 
        html += `<div class="leaderboard-row"><div style="font-weight:bold">#${i+1}</div><div class="user-cell"><img src="${x.avatar_url||'https://robohash.org/def'}"><span>${x.username||x.wallet_address.slice(0,4)}</span></div><div style="font-family:monospace">${x.points}</div></div>`; 
    });
    div.innerHTML = html;
}
