...
// Inicializa o Web3Auth silenciosamente ao carregar a página
window.onload = async function() {
    try {
        if(window.modal) { // Verifica se a biblioteca carregou
            ...
            console.log("Web3Auth Iniciado.");
        }
    } catch(e) { console.error("Erro Init Web3Auth:", e); }
};
...
window.connectWallet = async function(method) {
    try {
        ...
        // --- MÉTODO 2: SOCIAL (WEB3AUTH) ---
        else if (method === 'social') {
            if (!web3auth) return alert("Sistema de login carregando... tente em 5 segundos.");
            const web3authProvider = await web3auth.connect(); // Abre o modal do Google/X
            provider = new ethers.BrowserProvider(web3authProvider);
            signer = await provider.getSigner();
        }

        // PÓS-CONEXÃO (Igual para os dois)
        userAddress = await signer.getAddress();
        
        // Atualiza UI
        document.getElementById("btnConnect").style.display = 'none';
        document.getElementById("btnSocial").innerText = `🟢 Conectado: ${userAddress.slice(0,6)}...`;
        document.getElementById("btnSocial").classList.add('btn-disconnect');
