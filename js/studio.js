...
function renderStudioUI() {
    ...
    const tools = document.getElementById('layerTools'); // BARRA DE FERRAMENTAS

    // Render Lista
    if(window.layers.length === 0) {
        list.innerHTML = `<div style="padding:20px; text-align:center; color:#666; font-size:0.8rem;">Clique em +</div>`;
    } else {
        let html = "";
        window.layers.forEach((l, i) => {
            const isActive = i === window.activeLayerIndex ? 'active' : '';
            html += `<div class="layer-item ${isActive}" onclick="selectLayer(${i})"><span style="font-weight:500">${i+1}. ${l.name}</span><div style="display:flex; align-items:center; gap:8px;"><span class="layer-count">${l.traits.length}</span><i data-lucide="trash-2" style="width:14px; cursor:pointer; color:#666;" onclick="deleteLayer(${i}, event)"></i></div></div>`;
        });
        list.innerHTML = html;
    }
    ...
    if(window.activeLayerIndex === null) {
        ...
        emptyState.style.display = 'flex'; // Exibe o empty state
        tools.style.display = 'none'; // ESCONDE O BOTÃO DE IA
        title.innerText = "Studio";
        count.innerText = "";
    } else {
        grid.style.display = 'grid';
        emptyState.style.display = 'none';
        tools.style.display = 'flex'; // MOSTRA O BOTÃO DE IA
        ...
        currentLayer.traits.forEach((t, tIndex) => {
            gridHtml += `<div class="trait-card"><button class="btn-delete-trait" onclick="deleteTrait(${tIndex})"><i data-lucide="x" style="width:14px;"></i></button><div class="trait-img-box"><img src="${t.file}"></div><div class="trait-info"><div class="trait-name" title="${t.name}">${t.name}</div><div style="display:flex; justify-content:space-between; color:#666; font-size:0.7rem;"><span>Rarity</span><span id="weightVal_${tIndex}">${t.weight}%</span></div><input type="range" class="trait-slider" min="1" max="100" value="${t.weight}" oninput="document.getElementById('weightVal_${tIndex}').innerText = this.value + '%'" onchange="updateTraitWeight(${tIndex}, this.value)"></div></div>`;
        });
...
function addTraitToLayer(base64Data, name) {
    ...
    window.layers[window.activeLayerIndex].traits.push({ file: base64Data, name: name, weight: 50, imgObj: img });
    renderStudioUI();
    studioGeneratePreview();
}

window.deleteTrait = function(traitIndex) { window.layers[window.activeLayerIndex].traits.splice(traitIndex, 1); renderStudioUI(); studioGeneratePreview(); }
window.updateTraitWeight = function(tIndex, val) { window.layers[window.activeLayerIndex].traits[tIndex].weight = parseInt(val); }
...
window.runAiGeneration = function() {
    ...
    img.onload = function() { btn.innerText = "Gerar Imagem"; btn.disabled = false; area.style.display = 'block'; };
    img.onerror = function() { alert("Erro ao gerar imagem."); btn.innerText = "Gerar Imagem"; btn.disabled = false; };
    img.crossOrigin = "Anonymous"; 
    img.src = url;
}
...
window.useAiImage = function(imgElement) {
    ...
    try {
        ...
        addTraitToLayer(dataUrl, "AI_Gen_" + Date.now());
        document.getElementById('aiDialog').close();
    } catch(e) { alert("Erro CORS. Tente salvar e fazer upload manual."); }
}
...
window.studioGeneratePreview = function() {
    ...
    window.layers.forEach(layer => {
        if(layer.traits.length > 0) {
            ...
            if(trait && trait.imgObj) ctx.drawImage(trait.imgObj, 0, 0, canvas.width, canvas.height);
        }
    });
}
...
function pickWeighted(traits) {
    ...
    for(const t of traits) { if(r < t.weight) return t; r -= t.weight; }
    return traits[0];
}

window.studioStartGeneration = async function() {
    if(window.layers.length === 0) return alert("Adicione camadas!");
